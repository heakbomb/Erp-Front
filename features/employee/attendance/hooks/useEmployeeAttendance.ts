// features/employee/attendance/hooks/useEmployeeAttendance.ts
"use client";
...
import {
  ...
  punchAttendance,
} from "@/features/employee/attendance/services/attendanceService";
...
export function useEmployeeAttendance(initialEmployeeId = "3", initialStoreId = "11") {
  ...
  const loadRecent = useCallback(async () => {
    ...
    try {
      ...
      const data = await fetchRecentAttendance(Number(employeeId), Number(storeId));
      setRecent(data);
      setPage(1);
    } finally {
  ...
  const loadDay = useCallback(
    async (d: Date) => {
      ...
      try {
        setLoadingDay(true);
        const data = await fetchDayAttendance(
          Number(employeeId),
          Number(storeId),
          isoDate(d)
        );
