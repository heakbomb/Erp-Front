// features/employee/attendance/mobile/hooks/useMobilePunch.ts
"use client";

import { useCallback, useEffect, useMemo, useState } from "react";
import {
  fetchRecentMobileAttendance,
  punchMobileAttendance,
  MobileAttendanceItem,
} from "@/features/employee/attendance/mobile/services/mobileAttendanceService";
...
export function useMobilePunch(opts?: { employeeId?: number; storeId?: number }) {
  ...
  const fetchRecent = useCallback(async () => {
    try {
      const data = await fetchRecentMobileAttendance(employeeId, storeId);
