...
// ⭐️ 훅에서 가져오도록 수정
import { cancellationReasons } from "./hooks/useSubscriptionCancel"
...
export default function SubscriptionCancelPage() {
  const {
    ...
    handleConfirmCancel,
    isCanceling,
    // ⭐️ (확장) API에서 취소 정보를 가져올 수 있음
    // currentPlanEndDate 
  } = useSubscriptionCancel()

  const planEndDate = "2024년 5월 15일"; // ⭐️ (임시) 훅에서 가져와야 함
          ...
          <CardContent className="space-y-4">
            <div className="rounded-lg bg-destructive/10 p-4 space-y-2">
              ...
              <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground">
                <li>현재 결제 주기 종료일({planEndDate})까지 서비스를 이용할 수 있습니다</li>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <Button
            ...
            disabled={!selectedReason || isCanceling}
          >
            {isCanceling && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            구독 취소 진행
          </Button>
